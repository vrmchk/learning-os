...
1. If the repo has a remote, `git pull --ff-only`. Report if it fails; do not
   proceed on a dirty or diverged tree without telling the user.
...
3. Propose a specific concept or cluster and **say which rule chose it**:
   ...
   3. untaught gaps — these redirect to `teach` or `study`, not `interview`
   4. the focus cluster, if `ROADMAP.md` sets one
   5. the lowest-level concepts in the focus topic
   6. the cluster untouched the longest
...
- If three consecutive answers land at L1, stop the session and switch to
  teaching.
...
- Mastery is the only place a level lives. The queue schedules; it carries no
  levels.
- Gaps are marked `taught` or `regressed`, never deleted.
