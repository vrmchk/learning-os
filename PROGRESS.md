# Progress

Generated 2026-09-13 by `scripts/progress.ps1`. Do not edit — regenerate.

**Focus:** .NET · cluster: none

## Calibration checkpoint

Interview sessions: **2 / 5**.
Self-flagged-weak answers graded L3+: **0**.

## Softness — last 2 interview sessions

| Session | Date | Target avg | Awarded avg | Gap | Disputes |
|---|---|---|---|---|---|
| [[2026-09-08-gc-generations]] | 2026-09-08 | 2.70 | 0.30 | 2.40 | 0 |
| [[2026-09-12-gc-triggers-and-budgets]] | 2026-09-12 | 2.80 | 1.60 | 1.20 | 0 |

Older half → newer half: target 2.70 → 2.80 (+0.10), awarded 0.30 → 1.60 (+1.30).

## .NET

90 concepts · average **L0.06** · last evidence 2026-09-12

| L0 | L1 | L2 | L3 | L4 | L5 |
|---|---|---|---|---|---|
| 85 | 5 | 0 | 0 | 0 | 0 |

| Cluster | Concepts | Avg | L0 | ≥L3 | Last evidence |
|---|---|---|---|---|---|
| Memory and GC | 13 | L0.38 | 8 | 0 | 2026-09-12 |
| C# language internals | 10 | L0.00 | 10 | 0 | — |
| Async and threading | 12 | L0.00 | 12 | 0 | — |
| Concurrency | 10 | L0.00 | 10 | 0 | — |
| Runtime and type system | 9 | L0.00 | 9 | 0 | — |
| Performance and diagnostics | 11 | L0.00 | 11 | 0 | — |
| Dependency injection and hosting | 8 | L0.00 | 8 | 0 | — |
| Data access and EF Core internals | 11 | L0.00 | 11 | 0 | — |
| HTTP, networking, and resilience | 6 | L0.00 | 6 | 0 | — |

**Gaps:** open 7 · studying 0 · taught 7 · verified 0 · regressed 0

## Review queue

6 scheduled · **4 overdue** · 2 due in the next 7 days.

| Concept | Topic | Due | Days overdue |
|---|---|---|---|
| [[gc-triggers-and-budgets]] | dotnet | 2026-09-13 | 0 |
| [[finalization-and-freachable-queue]] | dotnet | 2026-09-13 | 0 |
| [[stack-vs-heap-layout]] | dotnet | 2026-09-13 | 0 |
| [[boxing]] | dotnet | 2026-09-13 | 0 |

Due soon: [[gc-generations]] 2026-09-14 · [[large-object-heap]] 2026-09-14

## Activity

| Window | Interview | Teach | Study | Review |
|---|---|---|---|---|
| Last 7 days | 2 | 5 | 0 | 0 |
| Last 30 days | 2 | 5 | 0 | 0 |
| All time | 2 | 5 | 0 | 0 |

Excursions: 0 of 2 interview sessions.
Last session: 2026-09-13 — teach — large-object-heap.
Last weekly review: none.
